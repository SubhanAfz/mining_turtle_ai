...
CC: Tweaked's speaker peripheral provides a powerful way to play any audio you like with the [`speaker.playAudio`](../peripheral/speaker.md#v:playAudio)
...
Modulation](https://en.wikipedia.org/wiki/Pulse-code_modulation) (PCM). PCM can be thought of as the "standard" audio format, as it's incredibly easy to work with. For
...
Let's now look at a quick example. We're going to generate a [Sine Wave](https://en.wikipedia.org/wiki/Sine_wave) at 220Hz, which sounds like a low monotonous
...
passed off to [`speaker.playAudio`](../peripheral/speaker.md#v:playAudio) when the time is right. This allows us to build a *stream* of audio, where we read
...
rather odd loop with [`speaker.playAudio`](../peripheral/speaker.md#v:playAudio) and [`os.pullEvent`](../module/os.md#v:pullEvent).

Let's talk about this loop, why do we need to keep calling [`speaker.playAudio`](../peripheral/speaker.md#v:playAudio)? Remember that what we're trying to do
here is avoid keeping too much audio in memory at once. However, if we're generating audio quicker than the speakers can
play it, we're not helping at all - all this audio is still hanging around waiting to be played!

In order to avoid this, the speaker rejects any new chunks of audio if its backlog is too large. When this happens,
[`speaker.playAudio`](../peripheral/speaker.md#v:playAudio) returns false. Once enough audio has played, and the backlog has been reduced, a
[`speaker_audio_empty`](../event/speaker_audio_empty.md) event is queued, and we can try to play our chunk once more.
...
OpenComputers) world. Originally popularised by the addon mod [Computronics](https://github.com/Vexatos/Computronics/), CC:T now has built-in support for it with
the [`cc.audio.dfpwm`](../library/cc.audio.dfpwm.md) module. This allows you to read DFPWM files from disk, decode them to PCM, and then play them
using the speaker.
...
Once again, we see the [`speaker.playAudio`](../peripheral/speaker.md#v:playAudio)/[`speaker_audio_empty`](../event/speaker_audio_empty.md) loop. However, the rest of the program is a little
different.

First, we require the dfpwm module and call [`cc.audio.dfpwm.make_decoder`](../library/cc.audio.dfpwm.md#v:make_decoder) to construct a new decoder. This decoder
accepts blocks of DFPWM data and converts it to a list of 8-bit amplitudes, which we can then play with our speaker.

As mentioned above, [`speaker.playAudio`](../peripheral/speaker.md#v:playAudio) accepts at most 128×1024 samples in one go. DFPWM uses a single bit for each
sample, which means we want to process our audio in chunks of 16×1024 bytes (16KiB). In order to do this, we use
[`io.lines`](../module/io.md#v:lines), which provides a nice way to loop over chunks of a file. You can of course just use [`fs.open`](../module/fs.md#v:open) and
[`fs.ReadHandle.read`](../module/fs.md#ty:ReadHandle:read) if you prefer.
...
[Ring Buffer](https://en.wikipedia.org/wiki/Circular_buffer), which helps makes things a little more efficient.
...
cover. That said, don't be afraid to ask [the community for help](/index.md#community).
...
For this, you'd need to use the [Fast Fourier transform](https://en.wikipedia.org/wiki/Fast_Fourier_transform) to convert the stream of amplitudes to frequencies,
...
- **[`speaker.playAudio`](../peripheral/speaker.md#v:playAudio)** Play PCM audio using a speaker.
- **[`cc.audio.dfpwm`](../library/cc.audio.dfpwm.md)** Provides utilities for encoding and decoding DFPWM files.
