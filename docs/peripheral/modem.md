...
[`rednet`](../module/rednet.md) API is built on top of modems, and provides a more user-friendly interface.

## Sending and receiving messages

Modems operate on a series of channels, a bit like frequencies on a radio. Any modem can send a message on a
particular channel, but only those which have [opened](modem.md#v:open) the channel and are "listening in" can receive
messages.

Channels are represented as an integer between 0 and 65535 inclusive. These channels don't have any defined meaning,
though some APIs or programs will assign a meaning to them. For instance, the [`gps`](../module/gps.md) module sends all its messages on
channel 65534 ([`gps.CHANNEL_GPS`](../module/gps.md#v:CHANNEL_GPS)), while [`rednet`](../module/rednet.md) uses channels equal to the computer's ID.

- Sending messages is done with the [`transmit`](modem.md#v:transmit) message.
- Receiving messages is done by listening to the [`modem_message`](../event/modem_message.md) event.
...
![Ender Pearl](https://tweaked.cc/images/items/minecraft/ender_pearl.png)
...
![Wireless Modem](https://tweaked.cc/images/items/computercraft/wireless_modem_normal.png)
...
![Eye of Ender](https://tweaked.cc/images/items/minecraft/ender_eye.png)
...
![Ender Modem](https://tweaked.cc/images/items/computercraft/wireless_modem_advanced.png)
...
![Networking Cable](https://tweaked.cc/images/items/computercraft/cable.png)6

**Wired Modem**

![Wired Modem](https://tweaked.cc/images/items/computercraft/wired_modem.png)

![Wired Modem](https://tweaked.cc/images/items/computercraft/wired_modem_full.png)
...
```
local modem = [peripheral.find](../module/peripheral.md#v:find)("modem") or error("No modem attached", 0)
...
repeat
 event, side, channel, replyChannel, message, distance = [os.pullEvent](../module/os.md#v:pullEvent)("modem_message")
until channel == 43
...
- **[`modem_message`](../event/modem_message.md)** Queued when a modem receives a message on an [open channel](modem.md#v:open).
- **[`rednet`](../module/rednet.md)** A networking API built on top of the modem peripheral.
...
[](#v:open)open(channel)[Source](https://github.com/cc-tweaked/CC-Tweaked/blob/db32ddfec5e8c2bdefb3232b471328a3e92cc43f/projects/common/src/main/java/dan200/computercraft/shared/peripheral/modem/ModemPeripheral.java#L160)
...
[](#v:isOpen)isOpen(channel)[Source](https://github.com/cc-tweaked/CC-Tweaked/blob/db32ddfec5e8c2bdefb3232b471328a3e92cc43f/projects/common/src/main/java/dan200/computercraft/shared/peripheral/modem/ModemPeripheral.java#L172)
...
[](#v:close)close(channel)[Source](https://github.com/cc-tweaked/CC-Tweaked/blob/db32ddfec5e8c2bdefb3232b471328a3e92cc43f/projects/common/src/main/java/dan200/computercraft/shared/peripheral/modem/ModemPeripheral.java#L183)
...
[](#v:closeAll)closeAll()[Source](https://github.com/cc-tweaked/CC-Tweaked/blob/db32ddfec5e8c2bdefb3232b471328a3e92cc43f/projects/common/src/main/java/dan200/computercraft/shared/peripheral/modem/ModemPeripheral.java#L191)

Close all open channels.

[](#v:transmit)transmit(channel, replyChannel, payload)[Source](https://github.com/cc-tweaked/CC-Tweaked/blob/db32ddfec5e8c2bdefb3232b471328a3e92cc43f/projects/common/src/main/java/dan200/computercraft/shared/peripheral/modem/ModemPeripheral.java#L217)
...
`channel` or entirely different. The channel must have been [opened](modem.md#v:open) on
...
[](#v:isWireless)isWireless()[Source](https://github.com/cc-tweaked/CC-Tweaked/blob/db32ddfec5e8c2bdefb3232b471328a3e92cc43f/projects/common/src/main/java/dan200/computercraft/shared/peripheral/modem/ModemPeripheral.java#L244)
...
[](#v:getNamesRemote)getNamesRemote()[Source](https://github.com/cc-tweaked/CC-Tweaked/blob/db32ddfec5e8c2bdefb3232b471328a3e92cc43f/projects/common/src/main/java/dan200/computercraft/shared/peripheral/modem/wired/WiredModemPeripheral.java#L97)
...
This function only appears on wired modems. Check [`isWireless`](modem.md#v:isWireless) returns false before calling it.

### Returns

1. { [`string`](https://www.lua.org/manual/5.1/manual.html#5.4)... } Remote peripheral names on the network.

[](#v:isPresentRemote)isPresentRemote(name)[Source](https://github.com/cc-tweaked/CC-Tweaked/blob/db32ddfec5e8c2bdefb3232b471328a3e92cc43f/projects/common/src/main/java/dan200/computercraft/shared/peripheral/modem/wired/WiredModemPeripheral.java#L114)
...
- **[`peripheral.isPresent`](../module/peripheral.md#v:isPresent)**

[](#v:getTypeRemote)getTypeRemote(name)[Source](https://github.com/cc-tweaked/CC-Tweaked/blob/db32ddfec5e8c2bdefb3232b471328a3e92cc43f/projects/common/src/main/java/dan200/computercraft/shared/peripheral/modem/wired/WiredModemPeripheral.java#L132)
...
- **[`peripheral.getType`](../module/peripheral.md#v:getType)**

### Changes

- **Changed in version 1.99:** Peripherals can have multiple types - this function returns multiple values.

[](#v:hasTypeRemote)hasTypeRemote(name, type)[Source](https://github.com/cc-tweaked/CC-Tweaked/blob/db32ddfec5e8c2bdefb3232b471328a3e92cc43f/projects/common/src/main/java/dan200/computercraft/shared/peripheral/modem/wired/WiredModemPeripheral.java#L152)
...
[](#v:getMethodsRemote)getMethodsRemote(name)[Source](https://github.com/cc-tweaked/CC-Tweaked/blob/db32ddfec5e8c2bdefb3232b471328a3e92cc43f/projects/common/src/main/java/dan200/computercraft/shared/peripheral/modem/wired/WiredModemPeripheral.java#L170)
...
- **[`peripheral.getMethods`](../module/peripheral.md#v:getMethods)**

[](#v:callRemote)callRemote(remoteName, method, ...)[Source](https://github.com/cc-tweaked/CC-Tweaked/blob/db32ddfec5e8c2bdefb3232b471328a3e92cc43f/projects/common/src/main/java/dan200/computercraft/shared/peripheral/modem/wired/WiredModemPeripheral.java#L195)
...
- **[`peripheral.call`](../module/peripheral.md#v:call)**

[](#v:getNameLocal)getNameLocal()[Source](https://github.com/cc-tweaked/CC-Tweaked/blob/db32ddfec5e8c2bdefb3232b471328a3e92cc43f/projects/common/src/main/java/dan200/computercraft/shared/peripheral/modem/wired/WiredModemPeripheral.java#L217)
