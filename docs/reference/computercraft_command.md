...
- All players have access to the [`queue`](#queue) sub-command.
- On a multi-player server, all other commands require op.
- On a single-player world, the player can run the [`dump`](#dump), [`turn-on`](#turn-on)/[`shutdown`](#shutdown), and [`track`](#track) sub-commands, even
when cheats are not enabled. The [`tp`](#tp) and [`view`](#view) commands require cheats.

If a permission mod such as [LuckPerms](https://github.com/LuckPerms/LuckPerms/) is installed[1](#fn-permission), you can configure access to the individual
...
Some commands (such as [`tp`](#tp) or [`turn-on`](#turn-on)) target a specific computer, or a list of computers. To specify which
computers to operate on, you must use "computer selectors".

Computer selectors are similar to Minecraft's [entity target selectors](https://minecraft.wiki/w/Target_selectors), but targeting computers instead. They allow
...
![A screenshot of a Minecraft world. In the chat box, there is a table listing 5 computers, with columns labelled "Computer", "On" and "Position". Below that, is a more detailed list of information about Computer 0, including its label ("My computer") and that it has a monitor on the right hand side](https://tweaked.cc/computercraft-dump-e4816091.png)
...
### [](#turn-on)`/computercraft turn-on [computers...]`
...
### [](#shutdown)`/computercraft shutdown [computers...]`
...
### [](#tp)`/computercraft tp [computer]`

Teleport to the given computer.

This is normally used from via the [`dump`](#dump) command interface rather than being invoked directly.

### [](#view)`/computercraft view [computer]`
...
### [](#queue)`/computercraft queue`

The queue subcommand allows non-operator players to queue a `computer_command` event on *command* computers.

This has a similar purpose to vanilla's [`/trigger`](https://minecraft.wiki/w/Commands/trigger) command. Command computers may choose to listen to this event, and
then perform some action.

1. 

This supports any mod which uses Forge's permission API or [fabric-permission-api](https://github.com/lucko/fabric-permissions-api). [↩︎︎](#ref-1-fn-permission)
